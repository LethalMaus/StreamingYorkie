        ...
        try {
            ...
            if (response.has("follows")) {
                ...
                for (int i = 0; i < response.getJSONArray("follows").length(); i++) {
                    ...
                    new WriteFileHandler(weakContext, userPath + File.separator + userObject.getString("_id"), null, userObject.toString(),false).run();
                    userIDs.add(response.getJSONArray("follows").getJSONObject(i).getJSONObject(arrayPath).getString("_id"));
                }
                new WriteFileHandler(weakContext, requestDestinationPath, userIDs, null, false).run();
            } else {
                userObject = new JSONObject();
                ...
                new WriteFileHandler(weakContext, userPath + File.separator + userObject.getString("_id"), null, userObject.toString(),false).run();
                new WriteFileHandler(weakContext, requestDestinationPath + File.separator + userObject.getString("_id"), null, null,false).run();
            }
